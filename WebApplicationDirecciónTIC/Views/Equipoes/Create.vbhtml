...
@Using (Html.BeginForm()) 
    ...
    @<div class="form-horizontal">
        ...
        <div class="form-group">
            ...
            <div class="col-md-10">
                @Html.DropDownList("PersonaId", Nothing, htmlAttributes:= New With { .class = "form-control" })
                @Html.ValidationMessageFor(Function(model) model.PersonaId, "", New With { .class = "text-danger" })
            </div>
        </div>

        <div class="form-group">
            @Html.LabelFor(Function(model) model.EstadoId, "EstadoId", htmlAttributes:= New With { .class = "control-label col-md-2" })
            <div class="col-md-10">
                @Html.DropDownList("EstadoId", Nothing, htmlAttributes:= New With { .class = "form-control" })
                @Html.ValidationMessageFor(Function(model) model.EstadoId, "", New With { .class = "text-danger" })
            </div>
        </div>
